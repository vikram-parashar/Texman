...
app.post("/register", async (req, res) => {
    ...
    try {
        ...
        if (user!==null) {
            ...
            carts.create({ userName:userName,password:hashed,productArray:null});
            Users.create({ userName: userName, password: hashed, email: email }).then((createdUser) => {
...
app.put("/cart/:id",validateToken,async(req,res)=>{
    ...
    try {
        ...
        await carts.updateOne({userName:user.userName},{ $push: { productArray: id } }).then(
            res.status(200)
        )
...
app.delete("/cart/:id",validateToken,async(req,res)=>{
    ...
    try {
        const user = await Users.findById(Uid);
        await carts.updateOne({userName:user.userName},{ $pull: { productArray: pid } })
        .then(res.status(200));
